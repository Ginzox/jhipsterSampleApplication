...
@Component({
    selector: 'jhi-registro-my-suffix',
    templateUrl: './registro-my-suffix.component.html'
})
export class RegistroMySuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.registroService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<RegistroMySuffix[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/registro-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/registro-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: RegistroMySuffix) {
        return item.id;
    }
